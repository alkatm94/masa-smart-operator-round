...
import { analyseGaugeDetailed, isNumericStable, matchEquipment, type VisionDetection, type VisionQuality } from "@/lib/vision";
...
export function OperatorVisionScan({ round, settings, calibrations, back, finish }: { round: Round; settings: AppState["settings"]; calibrations: AppState["calibrations"]; back: () => void; finish: (session: ScanSession) => void }) {
  ...
  const generalDetections = useRef<VisionDetection[]>([]), gaugeProposal = useRef<{ box: VisionDetection["box"]; frames: number } | null>(null);
  ...
  const process = useCallback(async () => {
    ...
    try {
      ...
      if ((!industrial.current?.isReady() || !candidates.length) && now - lastGeneral.current > 1000) { lastGeneral.current = now; const objects = await general.current?.detect(target) || []; generalDetections.current = objects; activePipelines.push("general"); const obstructions = objects.filter((item) => /person|backpack|handbag|suitcase|bottle|chair/i.test(item.label) && isPossibleObstruction(item.box)).map((item) => ({ type: "object_obstruction" as const, label: "Possible obstruction", confidence: item.confidence, bbox: item.box, statusText: `Possible obstruction: ${item.label}`, source: "general" })); publish(obstructions); }
      const gaugeCandidate = candidates.find((item) => item.metadata?.industrialClass === "analog_gauge");
      if (now - lastGauge.current > 750) { lastGauge.current = now; const roiCanvas = gaugeCandidate ? crop(target, gaugeCandidate.box) : target; const analysis = analyseGaugeDetailed(roiCanvas.getContext("2d", { willReadFrequently: true })!.getImageData(0, 0, roiCanvas.width, roiCanvas.height), calibrations.find((item) => item.stationId === round.stationId), gaugeCandidate ? { candidateBox: gaugeCandidate.box, suppressions: generalDetections.current } : { fullFrame: true, suppressions: generalDetections.current }); const result = analysis.detections[0]; if (!result) { gaugeProposal.current = null; setStats((s) => ({ ...s, gaugeConfidence: 0 })); } else { const box = gaugeCandidate?.box || result.box, previous = gaugeProposal.current, compatible = previous && Math.hypot(box.x - previous.box.x, box.y - previous.box.y) < .06 && Math.abs(box.width - previous.box.width) < .08; gaugeProposal.current = { box, frames: compatible ? previous.frames + 1 : 1 }; if (gaugeProposal.current.frames >= 3) { const calibration = calibrations.find((item) => item.stationId === round.stationId); publish([{ type: "analog_gauge", label: calibration ? "Pressure Gauge" : "Gauge detected", confidence: result.confidence, bbox: box, value: result.value, unit: calibration?.unit, statusText: calibration ? undefined : "Calibration required", source: "gauge" }]); setStats((s) => ({ ...s, gaugeConfidence: result.confidence })); activePipelines.push("gauge"); } } }
