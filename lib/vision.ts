...
export function needleDirectionScore(m: Omit<RadialMetrics, "score">) {
  return clamp(
    m.centerPassScore * 0.1 +
      m.lengthScore * 0.16 +
      m.thinnessScore * 0.16 +
      m.outerReachScore * 0.2 +
      m.continuityScore * 0.18 +
      m.radialDarknessScore * 0.2 -
      m.counterweightPenalty * 0.22 -
      m.tickMarkPenalty * 0.12,
  );
}
    ...
    for (let ox = -2; ox <= 2; ox++)
      for (let ri = 0; ri < 5; ri++) {
        ...
        for (let deg = 0; deg < 360; deg += 12) {
          ...
          const e = Math.abs(inside - outside);
          edge += e;
          if (e > 16) covered++;
        }
        const score = clamp(edge / (30 * 48)) * 0.55 + (covered / 30) * 0.45;
...
function radialMetrics(
  ...
  let dark = 0,
    ...
    outerOnly = 0,
    samples = 0;
  for (let t = 0.18; t <= 0.92; t += 0.025) {
    const x = cx + Math.cos(rad) * r * t,
      y = cy + Math.sin(rad) * r * t,
      center = pixel(gray, w, h, x, y),
      side2 =
        (pixel(gray, w, h, x + px * 2, y + py * 2) +
          pixel(gray, w, h, x - px * 2, y - py * 2)) /
        2,
      side5 =
        (pixel(gray, w, h, x + px * 5, y + py * 5) +
          pixel(gray, w, h, x - px * 5, y - py * 5)) /
        2,
      darkness = clamp((Math.min(faceMean, side5) - center) / 70),
      isLine = darkness > 0.13;
    dark += darkness;
    samples++;
    if (isLine) {
      continuous++;
      ...
      reach = t;
      if (center + 8 < side2 && side2 + 5 < side5) thin++;
      if (t < 0.42 && side5 < faceMean - 18) wide++;
    } else continuous = 0;
  ...
  const continuityScore = longest / Math.max(1, samples),
    ...
    thinnessScore = thin / Math.max(1, samples),
    radialDarknessScore = dark / Math.max(1, samples),
    centerPassScore = clamp((radialDarknessScore + continuityScore) * 0.8),
    counterweightPenalty = clamp(wide / (samples * 0.28)),
...
function digitalScore(
  ...
  for (let y = Math.floor(h * 0.25); y < h * 0.75; y += 3)
    for (let x = Math.floor(w * 0.18); x < w * 0.82; x += 3) {
      const p = y * w + x;
      horizontal += Math.abs(gray[p - 1] - gray[p + 1]);
      vertical += Math.abs(gray[p - w] - gray[p + w]);
      if (gray[p] < faceMean - 25) dark++;
      count++;
    }
  return clamp(
    clamp((horizontal + vertical) / (Math.max(1, count) * 45)) * 0.52 +
      clamp((dark / Math.max(1, count)) * 3) * 0.38 -
      circleScore * 0.28,
  );
}
export function analyseGauge(
  ...
  const selected = candidates[0],
    ...
    analogGaugeScore = clamp(
      circleScore * 0.42 + selected.score * 0.43 + tickScore * 0.15,
    ),
    digitalDisplayScore = digitalScore(gray, w, h, faceMean, circleScore);
  if (
    analogGaugeScore < 0.36 ||
    analogGaugeScore <= digitalDisplayScore + 0.04 ||
    selected.score < 0.2
  )
    return [];
