...
export const INDUSTRIAL_MODEL_CLASSES = ["pump", "motor", "analog_gauge", "digital_meter", "valve", "handwheel", "control_panel", "generator", "tank", "pipe", "flange", "nameplate"] as const;
...
const priority = (d: VisionDetection) => d.source === "industrial" ? 5 : d.source === "gauge" ? 4 : d.source === "equipment-ocr" ? 3 : d.source === "digital-ocr" ? 2 : 1;
...
export function mergePipelineResults(results: PipelineResult[], iouThreshold = 0.5) {
  ...
  for (const detection of sorted) {
    if (kept.some((existing) => intersectionOverUnion(existing.box, detection.box) >= iouThreshold)) dropped++;
